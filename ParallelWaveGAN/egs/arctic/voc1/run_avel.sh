...
n_jobs=6      # number of parallel jobs in feature extraction

# NOTE(kan-bayashi): renamed to conf to avoid conflict in parse_options.sh
conf=conf/parallel_wavegan_concate_Wavlm_mel.yaml
# conf=conf/parallel_wavegan.wavlm.yaml
...
spid=NL01
# download_dir=/home/4TB_storage/hsinhao_storage/ParallelWaveGAN/pwg_data/${spid} # directory to save database
download_dir=/home/4TB_storage/hsinhao_storage/ParallelWaveGAN/pwg_aligned_nl_data/${spid} # directory to save database

# spid=wav
# download_dir=/home/4TB_storage/hsinhao_storage/ParallelWaveGAN/pwg_tmsv_data/${spid}

# training related setting
tag=${spid}     # tag for directory to save model
# resume=""
resume="/home/4TB_storage/hsinhao_storage/AVEL_align/cdvae-align-feature-sly-align/downloads/pwg_concate_feat_18tmsv/checkpoint-500000steps.pkl"  # checkpoint path to resume training
           # (e.g. <path>/<to>/checkpoint-10000steps.pkl)

# decoding related setting
# checkpoint="/home/4TB_storage/hsinhao_storage/AVEL_align/cdvae-align-feature-sly-align/downloads/pwg_NL01/checkpoint-400000steps.pkl"
...
train_set=${spid}"_train" # name of training data directory
dev_set=${spid}"_dev"           # name of development data directory
eval_set=${spid}"_eval"         # name of evaluation data directory
...
if [ "${stage}" -le 0 ] && [ "${stop_stage}" -ge 0 ]; then
    ...
    python gen_pwg_data.py -i ${download_dir} \
                           -nt ${num_train} \
                           -ne ${num_dev} \
                           -nd ${num_eval} \
                           -d ${data_dir}
...
if [ -z "${tag}" ]; then
    expdir="exp/${train_set}_elvc_$(basename "${conf}" .yaml)"
else
    expdir="exp/${train_set}_elvc_${tag}"
fi
