...
class GMMConvertor(object):
    ...
    def open_from_param(self, param, f0_mode=False):
        ...
        """
        self.param = param
        self._deploy_parameters(f0_mode)
        return

    def convert(self, data, cvtype='mlpg', f0mode=False):
        ...
        cseq, wseq, mseq, covseq = self._gmmmap(data, f0mode)
    ...
    def _gmmmap(self, sddata, f0mode=False):
    ...
    def _deploy_parameters(self, f0mode=False):
    ...
    def _set_Ab(self, f0_mode=False):
        ...
        if f0_mode:
            self.A = np.zeros((self.n_mix, 2, sddim))
            self.b = np.zeros((self.n_mix, 2))
            self.cond_cov_inv = np.zeros((self.n_mix, 2, 2))
        else:
