...
class Trainer(object):
    def __init__(self, config):
        ...
        freeze_mods    = config.get('freeze_mods', None)
        ...
        if freeze_mods is not None:
            for mod, param in self.model.named_parameters():
                if any(mod.startswith(key) for key in freeze_mods.split(',')):
