...
if [ -f ${data}/resize_face_feats.scp ]; then
  mkdir -p ${data}/.backup
  echo "$0: moving $data/feats.scp to $data/.backup"
  [ -f ${data}/resize_face_feats.scp ] && mv ${data}/resize_face_feats.scp ${data}/.backup
fi

scp=${data}/face_feats.scp
...
split_scps=""
for n in $(seq ${nj}); do
    split_scps="${split_scps} ${logdir}/face_feats.${n}.scp"
done
...
${cmd} JOB=1:${nj} ${logdir}/make_face_${name}.JOB.log \
    local/compute-face-feats.py \
        ...
        --compress=${compress} \
        --filetype ${filetype} \
        ${logdir}/face_feats.JOB.scp \
        ark,scp:${facedir}/resize_face_${name}.JOB.${ext},${facedir}/resize_face_${name}.JOB.scp \

# concatenate the .scp files together.
for n in $(seq ${nj}); do
    cat ${facedir}/resize_face_${name}.${n}.scp || exit 1;
done > ${data}/resize_face_feats.scp || exit 1
