...
def get_nlel_name(label_path):
    nlel_name = str(label_path)
    nlel_name = nlel_name.split('/')
    nlel_name = nlel_name[len(nlel_name)-1].split('.')[0]
    return nlel_name
...
def main(args):
    ...
    for utt_idx in list_pair:
        ...
        el_name = get_nlel_name(el_label_path)
        nl_name = get_nlel_name(nl_label_path)
        print(el_name)
        print(nl_name)
        print(el_label_path)
        print(nl_label_path)
        ...
        ### for plot interval
        EL_parse_timing.write('{} '.format(el_name))
        print(el_counter)
        print("el_label")
        print()
        for item in el_label:
            if item["text"] != "":
                ...
                nl_xmaxs.append(float(item["xmax"]))
                round_xmin = round(float(item["xmin"]),2)
                round_xmax = round(float(item["xmax"]),2)
                EL_parse_timing.write('{} {} {} '.format(item["text"],round_xmin,round_xmax))
        EL_parse_timing.write('\n')
        el_counter += 1

        NL_parse_timing.write('{} '.format(nl_name))
        print(nl_counter)
        print("nl_label")
        for item in nl_label:
            if item["text"] != "":
                ...
                NL_parse_timing.write('{} {} {} '.format(item["text"],item["xmin"],item["xmax"]))
        NL_parse_timing.write('\n')
        nl_counter += 1
