...
class Trainer(object):
    def __init__(self, train_config, model_config):
        ...
        if self.opt_param['optim_type'].upper() == 'RADAM':
            self.optimizer = RAdam( [{'params':self.model.el_mc.parameters()},
                                     {'params':self.model.el_sp.parameters()}], 
                                    lr=self.opt_param['learning_rate'],
                                    betas=(0.5,0.999),
                                    weight_decay=0.0)
        else:
            self.optimizer = torch.optim.Adam( [{'params':self.model.el_mc.parameters()},
                                                {'params':self.model.el_sp.parameters()}],
                                               lr=self.opt_param['learning_rate'],
                                               betas=(0.5,0.999),
                                               weight_decay=0.0)
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        if 'el_mc.layers.0.conv.weight' not in checkpoint_data['model'].keys():
...
class Model(torch.nn.Module):
    def __init__(self, arch):
        ...
        self.el_mc = Encoder(arch['encoder']['mcc'], arch['z_dim'])
        self.el_sp = Encoder(arch['encoder']['sp'], arch['z_dim'])
    ...
    def forward(self, input, encoder_kind='mcc', decoder_kind='mcc'):
        # Preprocess
        if self.training:
            x_elmc, x_elsp, x_nl, d_el, x_mel, y, pos = input
        ...
        if self.training:
            ...
            # EL Encode
            z_elmc = self.el_mc(x_elmc)
            z_elsp = self.el_sp(x_elsp)

            ### EL dtw align
            zh_elmc = self.get_dtwidx(z_vq, z_elmc, d_el)
            zh_elsp = self.get_dtwidx(z_vq, z_elsp, d_el)

            # EL VQ
            zh_elmc_norm = zh_elmc / zh_elmc.norm(dim=1, keepdim=True)
            zh_elmc_vq = zh_elmc_norm + (z_vq - zh_elmc_norm).detach()

            zh_elsp_norm = zh_elsp / zh_elsp.norm(dim=1, keepdim=True)
            zh_elsp_vq = zh_elsp_norm + (z_vq - zh_elsp_norm).detach()

            # EL Decode
            xh_elmc = self.decoder((zh_elmc_vq,y))
            xh_elsp = self.decoder((zh_elsp_vq,y))
            ...
            lat_loss_mc = (z_vq.detach() - zh_elmc_norm).pow(2).div(mean_factor).masked_select(mask).sum()
            lat_loss_mc += (zh_elmc - zh_elmc_norm).pow(2).div(mean_factor).masked_select(mask).sum()

            lat_loss_sp = (z_vq.detach() - zh_elsp_norm).pow(2).div(mean_factor).masked_select(mask).sum()
            lat_loss_sp += (zh_elsp - zh_elsp_norm).pow(2).div(mean_factor).masked_select(mask).sum()

            mel_loss_mc = (LOG_2PI + (xh_elmc - x_mel).pow(2)).div(2*mean_factor).masked_select(mask).sum()
            mel_loss_sp = (LOG_2PI + (xh_elsp - x_mel).pow(2)).div(2*mean_factor).masked_select(mask).sum()

            lat_loss = lat_loss_mc + lat_loss_sp
            mel_loss = mel_loss_mc + mel_loss_sp
            ...
            losses = {
                    'Total': loss.item(),
                    'Latent loss mc': lat_loss_mc.item(),
                    'Latent loss sp': lat_loss_sp.item(),
                    'Latent loss': lat_loss.item(),
                    'Mel loss mc': mel_loss_mc.item(),
                    'Mel loss sp': mel_loss_sp.item(),
    ...
    def get_dtwidx(self, z_vq, z_el, d_el):
        # EL dtw align
        B, D, T_nl = z_vq.size()
        T_el = z_el.size(-1)
...
def collate(data):
    ...
    x_elmc, x_elsp, x_nl, d_el, x_mel, y, p_el, _, p_nl, _, _ = zip(*data)
    ...
    x_elmc = torch.cat([x[...,:max_length].unsqueeze(0) for x in x_elmc],dim=0)
    x_elsp = torch.cat([x[...,:max_length].unsqueeze(0) for x in x_elsp],dim=0)
    ...
    return x_elmc, x_elsp, x_nl, d_el, x_mel, y, p_nl


    # x_elmc, x_elsp, x_nl, d_el, x_mel, y, p_el, p_nl, _, _ = zip(*data)
    # aa, bb, cc, dd, ee, ff = zip(*data)
