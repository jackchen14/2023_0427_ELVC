...
# Load Config.
str_config = "conf/config_stage1_roy_cdvqvae_vc.json"
...
model_path = "exp/checkpoints_stage1_roy/06-22_13-00_600000"
print(model_path)
...
class Model(torch.nn.Module):
    ...
    def forward(self, input, encoder_kind='mcc', decoder_kind='mcc'):
        ...
        if self.training:
            # Encode to latent
            z_roy_mc = self.roy_encoder(dtw_el_mcc) ## roy latent
            with torch.no_grad():
                z_org_mc = org_encoder(dtw_nl_mcc) ## org latent
            ## decode roy latent
            with torch.no_grad():
                xh_mcc = org_decoder((z_roy_mc,y))

            batch_size = dtw_el_mcc.size(0)

            L1_Loss = torch.nn.L1Loss(size_average= False)
            latent_loss = L1_Loss(z_roy_mc, z_org_mc) / batch_size
            mc_reconstruction_loss = log_loss(xh_mcc, dtw_nl_mcc) / batch_size

            loss = latent_loss + mc_reconstruction_loss
            
            losses = {
                    'Total': loss.item(),
                    'latent_loss': latent_loss.item(),
                    'mc_reconstruction_loss': mc_reconstruction_loss.item(),
                }

            # assert org_spk_embeds == org_model.spk_embeds
            for p1, p2 in zip(org_decoder.parameters(), org_model.decoder['mcc'].parameters()):
                assert torch.equal(p1, p2)
            for p1, p2 in zip(org_encoder.parameters(), org_model.encoder['mcc'].parameters()):
                assert torch.equal(p1, p2)
            # for param in self.spk_embeds.parameters():
            #     print("check")
            #     print(param)
