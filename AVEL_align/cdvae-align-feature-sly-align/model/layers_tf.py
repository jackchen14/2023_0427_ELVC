...
class EncoderPrenet(nn.Module):
    ...
    """
    def __init__(self, num_mels, num_hidden):
...
class FFN_GLU_skip(nn.Module):
    ...
    def __init__(self, num_hidden):
        ...
        :param num_hidden: dimension of hidden 
        """
        super(FFN_GLU_skip, self).__init__()
        self.w_1 = Conv(num_hidden, num_hidden * 8, kernel_size=1, w_init='relu')
        self.w_2 = Conv(num_hidden * 4, num_hidden * 2, kernel_size=1)
        self.dropout = nn.Dropout(p=0.1)
        self.layer_norm = nn.LayerNorm(num_hidden)
        self.half_hidden = num_hidden * 4
        self.out_hidden = num_hidden

    def forward(self, input_):
        # FFN Network
        x = input_.transpose(1, 2) 
        x = self.w_1(x)
        x_t = t.tanh(x[:,:self.half_hidden,:])
        x_s = t.sigmoid(x[:,self.half_hidden:,:])
        x = self.w_2(x_t*x_s)
        x = x.transpose(1, 2) 

        # skip connection
        x_skip = x[:,:,:self.out_hidden]

        # residual connection
        x = x[:,:,self.out_hidden:] + input_ 
...
class DecoderPostConvNet(nn.Module):
    """
    Post Convolutional Network (mel --> mel) for decoder
    """
    def __init__(self, num_mels, num_hidden, outputs_per_step=1):
        """
        
        :param num_hidden: dimension of hidden 
        """
        super(DecoderPostConvNet, self).__init__()
...
class Attention(nn.Module):
    ...
    Attention Network
    """
    def __init__(self, num_hidden, num_condition=0, h=4, concat_after=False):
        """
        :param num_hidden: dimension of hidden
        ...
        self.key = Linear(num_hidden+num_condition, num_hidden, bias=False)
        self.value = Linear(num_hidden+num_condition, num_hidden, bias=False)
