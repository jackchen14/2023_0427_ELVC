...
from .layers_vq import ( VectorQuantizer, EncodeResidualStack, DecodeResidualStack)
...
class Trainer(object):
    ...
    def step(self, input, iteration=None):
        ...
        ################################
        if iteration > self.pre_iter and iteration % self.iter_per_G == 0:
            # Train the Generator
            x_real, x_fake, y_idx, loss, loss_detail_G = self.model_G((x_batch, y_batch))
...
class Model(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            # Convert
            y2_idx = torch.randperm(self.y_num,device=x.device)[:x.size(0)]
            y2_idx = y2_idx[:,None] * torch.ones_like(y_idx,device=x.device)
            y2 = self.embeds(y2_idx.long(),pre_norm=False).transpose(1,2).contiguous()    # Size( N, y_dim, 1)
            xhat2 = self.decoder((z_vq, y2))

            # Cycle
            # zhat = self.encoder(xhat)

            # Loss
            batch_size, z_dim, z_length = x.shape

            z_qut_loss = z_qut_loss / batch_size
            z_enc_loss = z_enc_loss / batch_size

            # z_cyc_loss = (zhat - z_vq).pow(2).sum() / batch_size
            
            x_loss = log_loss(xhat, x) / batch_size

            loss = x_loss + z_qut_loss + self.beta * z_enc_loss # + self.beta * z_cyc_loss
            
            losses = {'Total': loss.item(),
                      'VQ loss': z_enc_loss.item(),
                      'Entropy': entropy.item(),
                      # 'X cycle': z_cyc_loss.item(),
                      'X like': x_loss.item()}

            y_idx = torch.zeros_like(y_idx,device=y_idx.device)
            y_idx = y_idx[:,:1].repeat(1,1,x.size(-1))
    ...
    def __init__(self,
        ...
        for ( in_channel, out_channel, ds_scale, stack) in zip( in_channels, out_channels, downsample_scales, stacks):
            ...
            if ds_scale == 1:
                ...
                kernel_size = ds_scale * 2
                padding = ds_scale // 2 + ds_scale % 2
                stride = ds_scale

            layers += [
                getattr(torch.nn, pad)( padding, **pad_params),
                torch.nn.Conv1d(in_channel, out_channel, kernel_size, stride=stride, bias=bias)
            ]

            # add residual stack
            for j in range(stack):
                normalization_params["num_channels"] = out_channel
                layers += [
                    EncodeResidualStack(
...
class Decoder(torch.nn.Module):
    def __init__(self,
        ...
        # add final layer
        final_layer = DecodeResidualStack(
                            kernel_size=stack_kernel_size,
                            in_channels=skip_channels,
                            cond_channels=cond_channels,
                            skip_channels=final_channels,
                            dilation=1,
                            bias=True,
                            dropout=0.0,
                            nonlinear_activation=nonlinear_activation,
    ...
    def forward(self, input):
        ...
        for layer in self.layers:
            if isinstance(layer, DecodeResidualStack):
                copy_vec = torch.ones([x.size(-1), 1], dtype=torch.float, device=c.device)
                c_in = F.linear(c,copy_vec)
                x, x_skip = layer(x,c_in)
                x_out += x_skip
            else:
                x = layer(x)
        x = x_out * math.sqrt(1.0 / len(self.layers))
        _, x = self.final_layer(x,c)
        return x
