...
def inference_cross_domain(trials, utt2path, spk2spk_id,
              model_path,
    ...
    ## ap_mcc pwg
    if use_pwg and feat_kinds[-1] == 'CD_MelWavLM':
    ...
    for i, trial in enumerate(trials):
        ...
        if utt2path['type'] == 'feats':
            ...
            X_in_mel = [X["mel"].clone()]
            X_in_WavLM = [X["WavLM"].clone()]
           
        ## input mel   ---------------
        Y_in = torch.ones(1,1) * spk2spk_id[target_name]
        Y_in = Y_in.long().cuda()
        X_in_mel.append(Y_in)
        with torch.no_grad():
            Y_out_mel = model(X_in_mel , encoder_kind='mel', decoder_kind='mel')
            print(Y_out_mel.shape)

        Y_out_mel = mel_fn.denormalize({'mel':Y_out_mel})['mel']
        ## input WavLM   ---------------
        # Y_in = torch.ones(1,1) * spk2spk_id[target_name]
        # Y_in = Y_in.long().cuda()
        X_in_WavLM.append(Y_in)
        with torch.no_grad():
            Y_out_WavLM = model(X_in_WavLM , encoder_kind='WavLM', decoder_kind='WavLM')
            print(Y_out_WavLM.shape)

        Y_out_WavLM = WavLM_fn.denormalize({'WavLM':Y_out_WavLM})['WavLM']
        # # ipdb.set_trace()
        # # De-normalize
        ...
        if concate_feat_flag == True :
            ...
            WavLM_feat = Y_out_WavLM.cpu().detach().numpy()
            mel = torch.log10(torch.clamp(Y_out_mel, min=1e-10))
        ...
        if concate_feat_flag == True :
            with torch.no_grad():
                ...
                # concate_feat = torch.log10(torch.clamp(concate_feat, min=1e-10))
            ...
            with torch.no_grad():
                ...
                # Y_out_dn = torch.log10(torch.clamp(Y_out_dn, min=1e-10))
...
if __name__ == "__main__":
    ...
    inference_cross_domain(trials, utt2path, spk2spk_id,
              model_path,
              output_mcc_dir, output_wav_dir,
              model_type)
