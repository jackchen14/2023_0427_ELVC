...
for utt, path in feat_scp:
    ...
    for feat_pair in feature_kinds:
        ...
        for i in range(int((len(NL_parse_timing[temp_idx])-1)/3)):
            ...
            last_array = temp_dtwpath[1]
            last_EL_stop_time_idx = EL_stop_time_idx
        
        print("last_EL_stop_time_idx")   
        print(last_EL_stop_time_idx)
        print(last_array)
        # ipdb.set_trace()   
        print("dtwpath")
        ...
        remove_idx = []
        for i in range(len(wld_feature_dtwpath[1])):
            if (i!=0 and wld_feature_dtwpath[1][i] == 0):
                remove_idx.append(i)
        ...
        for i in range(length):
            if (wld_feature_dtwpath[1][count_a] == i + start) :
                complete_nl_array[i] = wld_feature_dtwpath[0][count_b]
                count_a += 1
                count_b += 1
            else :
                complete_nl_array[i] = math.floor((wld_feature_dtwpath[0][count_b-1] + wld_feature_dtwpath[0][count_b])/2)
