...
def mouse_pos(array1, array2):
    length = len(array1)
    dis = 0
    for i in range(length):
        dis += np.linalg.norm(array1[i]-array2[i])
    return dis
...
def estimate_twf(orgdata, tardata, distance='melcd', fast=True, otflag=None):
    ...
    if distance == 'melcd':
        ...
        def distance_func(x, y): return np.mean((y-x)**2)
    elif distance == 'mouth':
        def distance_func(x, y): return mouse_pos(x, y)
    else:
    ...
    if otflag is None:
        ...
        orgdata = np.array(orgdata, dtype='float')
        tardata = np.array(tardata, dtype='float')
        orgdata = orgdata.reshape(-1, 40)
        tardata = tardata.reshape(-1, 40)
        ...
        if otflag == 'org':
            #print(twf.shape)
            twf = twf[::-1, :]  # swap cols
            #print(twf.shape, orgdata.shape)
            assert twf.shape[1] == orgdata.shape[0]
        else:
            #print(twf.shape, tardata.shape)
...
def align_data(org_data, tar_data, twf):
    ...
    jdata = np.c_[np.array(org_data)[twf[0]], np.array(tar_data)[twf[1]]]
    return jdata
