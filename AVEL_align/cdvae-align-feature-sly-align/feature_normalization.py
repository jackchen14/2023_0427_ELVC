...
def main(config):
    ...
    if 'mel' in feature_kinds:
        ...
        if mel_fn.feat_stat:
            stat_kinds.append('mel')
    if 'WavLM' in feature_kinds:
        WavLM_fn = WavLM_function
        func_kinds.append(['WavLM', WavLM_fn])
    ...
    for data_name, data_path in data_list:
        ...
        for key, val in feat.items():
            ...
            if key in feature_kinds:
                # print("feature_kinds:")
                # print(feature_kinds)
                feat_norm[key] = val.unsqueeze(0) ## (1,3,3) (1,1,3,3) add one dimension


        for func_kind, feat_fn in func_kinds:
            # print("func_kinds func_kind feat_fn")
            # print(func_kinds)
            # print(func_kind)
            # print(feat_fn)
