...
def feature_to_audio():
    ...
    for utt, path in feat_scp:
        ...
        if output_wav_type == "nl" :
            audio_path = directory_path + "0829_aligned_wav/NL01/" + utt + ".wav"
        else :
            audio_path = directory_path + "0829_aligned_wav/EL01/" + utt + ".wav"
        yhat = yhat * MAX_WAV_VALUE
        wavfile.write( audio_path, sampling_rate, yhat.astype('int16'))

        ### save aligned feature
        if output_wav_type == "nl" :
            filepath_pkl = directory_path + "0909_aligned_feat/NL01/"+ utt + ".pkl"
        else :
            filepath_pkl = directory_path + "0909_aligned_feat/EL01/"+ utt + ".pkl"
        for i in feats :
            feats[i] = feats[i].cpu().detach().numpy()
        for i in feats :
            print(type(feats[i]))
        a_file = open(filepath_pkl, "wb")
        pickle.dump(feats, a_file)
        a_file.close()
